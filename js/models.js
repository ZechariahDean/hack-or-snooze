...
// $navSubmit.on("submit", storyList.addStory(currentUser, 
//   {'title':  $("#story-title"), 'author': $("#author-input"), 'url': $("#story-url")}));
...
class User {
  ...
  async _addOrRemFave(action, story) {
    console.debug("_addOrRemFave");
  ...
  async favorite(story) {
    ...
    await this._addOrRemFave("POST", story);
  }

  async unFavorite(story) {
    this.favorites = this.favorites.filter(stry => stry.storyId !== story.storyId);
    await this._addOrRemFave("DELETE", story);
  }

  // function to check if a story is favorited by a user
  isFaved(story) {
